...
#ifdef ENABLE_LOGGING
    log::debug({__func__, "created new isolate", __FILE__, __LINE__});
#endif
    v8::Isolate::Scope isolate_scope(isolate);
#ifdef ENABLE_LOGGING
    log::debug({__func__, "created isolate scope", __FILE__, __LINE__});
#endif
    v8::HandleScope handle_scope(isolate);
#ifdef ENABLE_LOGGING
    log::debug({__func__, "created handle scope", __FILE__, __LINE__});
#endif
    auto global_template = v8::ObjectTemplate::New(isolate);
#ifdef ENABLE_LOGGING
    log::debug({__func__, "created global_template", __FILE__, __LINE__});
#endif
    auto context = v8::Context::New(isolate, NULL, global_template);
#ifdef ENABLE_LOGGING
    log::debug({__func__, "created context", __FILE__, __LINE__});
#endif
    v8::Context::Scope context_scope(context);
#ifdef ENABLE_LOGGING
    log::debug({__func__, "created context scope", __FILE__, __LINE__});
#endif
...
#ifdef ENABLE_LOGGING
    log::debug({__func__, "registered dynamic_import_callback", __FILE__, __LINE__});
#endif
...
#ifdef ENABLE_LOGGING
    log::debug({__func__, "loaded module plugin", __FILE__, __LINE__});
#endif
    slim::plugin::loader::load_plugin(isolate, "path", true);
#ifdef ENABLE_LOGGING
    log::debug({__func__, "loaded path plugin", __FILE__, __LINE__});
#endif
...
#ifdef ENABLE_LOGGING
        log::debug({__func__, "resolve_imports returned empty", __FILE__, __LINE__});
        log::trace({__func__, "ends", __FILE__, __LINE__});
#endif
        return;
    }
#ifdef ENABLE_LOGGING
    log::debug({__func__, "has_value check passed, getting module_import_specifier", __FILE__, __LINE__});
#endif
    slim::module::import_specifier& module_import_specifier = module_import_specifier_optional.value().get();
#ifdef ENABLE_LOGGING
    log::debug({__func__, std::format("module status before evaluate => {}", static_cast<int>(module_import_specifier.v8_module()->GetStatus())), __FILE__, __LINE__});
#endif
...
#ifdef ENABLE_LOGGING
        log::debug({__func__, "module status is kErrored before evaluate", __FILE__, __LINE__});
        log::trace({__func__, "ends", __FILE__, __LINE__});
#endif
        return;
    }
...
#ifdef ENABLE_LOGGING
        log::debug({__func__, "calling Evaluate on context", __FILE__, __LINE__});
#endif
...
#ifdef ENABLE_LOGGING
        log::debug({__func__, std::format("Evaluate returned is_empty => {}", result.IsEmpty()), __FILE__, __LINE__});
#endif
...
#ifdef ENABLE_LOGGING
        log::debug({__func__, "stop requested, exiting keep alive loop", __FILE__, __LINE__});
#endif

        if(try_catch.HasCaught()) {
#ifdef ENABLE_LOGGING
            log::debug({__func__, std::format("Evaluate threw an exception => {}", utilities::v8StringToString(isolate, try_catch.Message()->Get())), __FILE__, __LINE__});
#endif
            slim::exception_handler::v8_try_catch_handler(&try_catch);
        }
#ifdef ENABLE_LOGGING
        log::debug({__func__, std::format("module status after evaluate => {}", static_cast<int>(module_import_specifier.v8_module()->GetStatus())), __FILE__, __LINE__});
#endif
        switch(module_import_specifier.v8_module()->GetStatus()) {
            case v8::Module::Status::kErrored:
#ifdef ENABLE_LOGGING
                log::debug({__func__, std::format("module exception after evaluate => {}", utilities::v8ValueToString(isolate, module_import_specifier.v8_module()->GetException())), __FILE__, __LINE__});
                log::debug({__func__, "module status is kErrored after evaluate", __FILE__, __LINE__});
#endif
                {
            ...
            default:
#ifdef ENABLE_LOGGING
                log::debug({__func__, "checking for stalled top level await modules", __FILE__, __LINE__});
#endif
...
#ifdef ENABLE_LOGGING
                    log::debug({__func__, "number of stalled top level await modules => "
                        + std::to_string(num_stalled_awaits), __FILE__, __LINE__});
                    log::debug({__func__, "number of stalled top level await messages => "
                        + std::to_string(await_message_module_pair.second.size()), __FILE__, __LINE__});
#endif
                }
                else {
#ifdef ENABLE_LOGGING
                    log::debug({__func__, "no stalled top level await objects", __FILE__, __LINE__});
#endif
                }
        }
    }
