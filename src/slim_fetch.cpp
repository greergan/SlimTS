...
SSL_CTX* get_ssl_ctx() {
    ...
    std::call_once(flag, []() {
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, "call_once: invoking SSL_CTX_new", __FILE__, __LINE__));
#endif
        ctx = SSL_CTX_new(TLS_client_method());
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, std::format("call_once: SSL_CTX_new => {}", (void*)ctx), __FILE__, __LINE__));
#endif
        if (ctx) {
            SSL_CTX_set_default_verify_paths(ctx);
#ifdef ENABLE_LOGGING
            log::debug(log::Message(__func__, "call_once: SSL_CTX_set_default_verify_paths done", __FILE__, __LINE__));
#endif
        } else {
#ifdef ENABLE_LOGGING
            log::error(log::Message(__func__, "call_once: SSL_CTX_new returned null", __FILE__, __LINE__));
#endif
        }
    });
#ifdef ENABLE_LOGGING
    log::debug(log::Message(__func__, std::format("returning ctx=>{}", (void*)ctx), __FILE__, __LINE__));
...
#ifdef ENABLE_LOGGING
    log::trace(log::Message(__func__, "begins", __FILE__, __LINE__));
    log::debug(log::Message(__func__, std::format("path=>'{}'", path), __FILE__, __LINE__));
#endif
    std::string resolved;
    try {
        ...
        resolved = p.string();
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, std::format("resolved=>'{}'", resolved), __FILE__, __LINE__));
#endif
    } catch (...) {
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, "exception resolving path", __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
    log::debug(log::Message(__func__, std::format("stat=>{}", stat_res), __FILE__, __LINE__));
#endif
    if (stat_res < 0) {
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, std::format("stat failed errno=>{}", -stat_op.result), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
    log::debug(log::Message(__func__, std::format("file size=>{} word_buf slots=>{}", stat_op.buf_.stx_size, word_buf.size()), __FILE__, __LINE__));
#endif

    Open open_op(scheduler, resolved, O_RDONLY);
    int fd = co_await open_op;
#ifdef ENABLE_LOGGING
    log::debug(log::Message(__func__, std::format("open => fd=>{}", fd), __FILE__, __LINE__));
#endif
    if (fd < 0) {
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, std::format("open failed errno=>{}", -open_op.result), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
    log::debug(log::Message(__func__, std::format("read => bytes=>{}", bytes_read), __FILE__, __LINE__));
#endif

    Close close_op(scheduler, fd);
    co_await close_op;
#ifdef ENABLE_LOGGING
    log::debug(log::Message(__func__, std::format("closed fd=>{}", fd), __FILE__, __LINE__));
#endif

    if (bytes_read < 0) {
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, std::format("read failed errno=>{}", -read_op.result), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
    log::debug(log::Message(__func__, std::format("returning 200 body_size=>{}", r.body.size()), __FILE__, __LINE__));
    log::trace(log::Message(__func__, "ends", __FILE__, __LINE__));
#endif
    co_return r;
}
...
#ifdef ENABLE_LOGGING
    log::trace(log::Message(__func__, "begins", __FILE__, __LINE__));
    log::debug(log::Message(__func__, std::format("url='{}' => ssl_ctx=>{} => redirects_left=>{}", url.href(), (void*)ssl_ctx, redirects_left), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
    log::debug(log::Message(__func__, std::format("host=>'{}' => port=>{}", host_str, port), __FILE__, __LINE__));
#endif

    try {
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, std::format("creating connection ssl=>{}", ssl_ctx ? "yes" : "no"), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, "connection established", __FILE__, __LINE__));
#endif

        Request req(url);
        co_await conn.write(req.serialize());
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, "request written", __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, std::format("raw response bytes=>{}", raw.size()), __FILE__, __LINE__));
#endif

        if (raw.empty()) {
#ifdef ENABLE_LOGGING
            log::error(log::Message(__func__, "raw response empty => 502", __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, std::format("parse => es=>{} => code=>{}", static_cast<int>(es), r.code), __FILE__, __LINE__));
#endif
        if (es != ErrorStatus::OK) {
#ifdef ENABLE_LOGGING
            log::error(log::Message(__func__, std::format("parse failed es=>{}", static_cast<int>(es)), __FILE__, __LINE__));
#endif
        ...
        if (redirects_left > 0 &&
...
#ifdef ENABLE_LOGGING
            log::debug(log::Message(__func__, std::format("redirect code=>{} => redirects_left=>{}", r.code, redirects_left), __FILE__, __LINE__));
#endif
            auto location_header = r.headers.get("Location");
            if (!location_header) {
#ifdef ENABLE_LOGGING
                log::error(log::Message(__func__, "redirect missing Location header => 502", __FILE__, __LINE__));
#endif
                co_return make_error(502);
            }
            ...
            if (values.empty()) {
#ifdef ENABLE_LOGGING
                log::error(log::Message(__func__, "redirect Location header empty => 502", __FILE__, __LINE__));
#endif
                co_return make_error(502);
            }
#ifdef ENABLE_LOGGING
            log::debug(log::Message(__func__, std::format("redirect location=>'{}'", values[0]), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
                log::error(log::Message(__func__, std::format("redirect URL parse failed: {}", e.what()), __FILE__, __LINE__));
#endif
                co_return make_error(error_status_to_http(e.error()));
            } catch (...) {
#ifdef ENABLE_LOGGING
                log::error(log::Message(__func__, "redirect URL parse unknown exception", __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
            log::debug(log::Message(__func__, std::format("following redirect to '{}' => ssl=>{}", next.href(), next_ssl ? "yes" : "no"), __FILE__, __LINE__));
#endif
            co_return co_await fetch_http(scheduler, next, next_ssl, redirects_left - 1);
        }

#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, std::format("returning code=>{} => body_size=>{}", r.code, r.body.size()), __FILE__, __LINE__));
        log::trace(log::Message(__func__, "ends", __FILE__, __LINE__));
#endif
        co_return r;
...
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, std::format("network exception: {}", e.what()), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, std::format("io exception: {}", e.what()), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, std::format("exception: {}", e.what()), __FILE__, __LINE__));
#endif
        auto r = make_error(500);
        r.code_text = e.what();
        co_return r;
...
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, "unknown exception", __FILE__, __LINE__));
#endif
        co_return make_error(500);
    }
}
...
#ifdef ENABLE_LOGGING
    log::trace(log::Message(__func__, std::format("begins uri=>'{}'", uri), __FILE__, __LINE__));
    log::debug(log::Message(__func__, std::format("uri=>'{}'", uri), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
        log::trace(log::Message(__func__, "post lambda begins", __FILE__, __LINE__));
#endif
        URL url;
        try {
            url = URL(uri);
#ifdef ENABLE_LOGGING
            log::debug(log::Message(__func__, std::format("{} => {} => hostname=>'{}' => pathname=>'{}'",
                uri, url.protocol(), url.hostname(), url.pathname()), __FILE__, __LINE__));
#endif
        } catch (const slim::common::http::UrlParseException& e) {
#ifdef ENABLE_LOGGING
            log::error(log::Message(__func__, std::format("URL parse failed: {}", e.what()), __FILE__, __LINE__));
#endif
            Response r;
...
#ifdef ENABLE_LOGGING
            log::error(log::Message(__func__, "URL parse unknown exception", __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
        log::debug(log::Message(__func__, std::format("{} => protocol=>'{}'", uri, protocol), __FILE__, __LINE__));
#endif

        if (protocol == "file") {
#ifdef ENABLE_LOGGING
            log::debug(log::Message(__func__, std::format("{} => dispatching to fetch_local", uri), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
            log::debug(log::Message(__func__, std::format("{} => dispatching to fetch_http ssl_ctx=>{}", uri, (void*)ctx), __FILE__, __LINE__));
#endif
...
#ifdef ENABLE_LOGGING
        log::error(log::Message(__func__, std::format("{} => unsupported protocol=>'{}'", uri, protocol), __FILE__, __LINE__));
#endif
        promise->set_value(make_error(ErrorStatus::UrlSchemeUnsupported));
    });

#ifdef ENABLE_LOGGING
    log::trace(log::Message(__func__, std::format("ends uri=>'{}'", uri), __FILE__, __LINE__));
#endif
    return future;
}
